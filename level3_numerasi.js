...
if (!nextLevelBtn) {
  ...
  nextLevelBtn.textContent = 'Ke Level 4';
...
if (!retryBtn) {
  ...
  retryBtn.textContent = 'Ulangi Level 3';
...
function showQuestion() {
  ...
  if (current >= questions.length) {
    ...
    localStorage.setItem('skor_level3', score);

    selesaiDiv.innerHTML = `
      <h2>Level 3 Selesai</h2>
      <p>Skor Akhir: <strong>${score}</strong></p>
    `;

    if (score >= 70) {
      localStorage.setItem('level3_numerasi_selesai', 'true');
      setTimeout(() => {
...
nextLevelBtn.onclick = () => {
  window.location.href = 'level4_numerasi.html';
};
retryBtn.onclick = () => {
  startGame();
};
