...
for (let i = 0; i < 5; i++) {
  const a = (Math.floor(Math.random() * 5) + 1) * 10 + (Math.floor(Math.random() * 5) + 1);
  const b = (Math.floor(Math.random() * 5) + 1) * 10 + (Math.floor(Math.random() * 5) + 1);
...
if (!nextLevelBtn) {
  ...
  nextLevelBtn.textContent = 'Ke Level 6';
...
if (!retryBtn) {
  ...
  retryBtn.textContent = 'Ulangi Level 5';
...
function showQuestion() {
  ...
  if (current >= questions.length) {
    ...
    localStorage.setItem('skor_level5', score);
    ...
    if (score >= 70) {
      localStorage.setItem('level5_numerasi_selesai', 'true');
      setTimeout(() => {
...
nextLevelBtn.onclick = () => {
  window.location.href = 'level6_numerasi.html';
};
retryBtn.onclick = () => {
  startGame();
};
